...
自动验证已通过：`npm test`（21 tests）、`npm run typecheck`、`npm run build`、`npm run manifest:validate`。
...
- [x] 容器实测为 3 行；双图列宽为 4873/4873 twip；每列自动带“图表 + 黑体加粗 SEQ Figure 自动编号 + 冒号”标题和“资料来源：”行；无边框、无图片提示词
...
从原 CICC Word 插件安装包中提取并检查了实际插入的占位图片，而非继续依据页面宽度猜测：

- 宽图：`638 × 201 px @ 96 dpi`，即 `9570 × 3015 twip`（6.646 × 2.094 英寸）；
- 窄图：`448 × 240 px @ 96 dpi`，即 `6720 × 3600 twip`（4.667 × 2.500 英寸），左缩进 `2100 twip`（原插件的 10 个中文字符单位）。

插件现用无边框空白容器保留这两个实际占位尺寸。宽图和窄图单元格边距为 0，避免空白容器再额外吞掉横向尺寸；双图继续使用 Coal India 标准稿的 `4873 / 4873 twip` 网格及每侧 `108 twip` 单元格边距。
