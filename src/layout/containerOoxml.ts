...
export function buildContainerOoxml(plan: ContainerPlan, pictureSlots = createPictureSlots(plan, "preview")): string {
  ...
  const rows = Array.from({ length: 3 }, (_, row) => `<w:tr>${row === 1 ? `<w:trPr><w:trHeight w:val="${plan.imageRowHeightTwip}" w:hRule="atLeast"/></w:trPr>` : ""}${plan.columnWidthsTwip.map((w, column) => cell(w, row, plan.imageAlignment, pictureSlots[column])).join("")}</w:tr>`).join("");
