...
const imageParagraph = (imageHeight: number, alignment: "left" | "center") => `<w:p><w:pPr><w:jc w:val="${alignment}"/><w:spacing w:line="${imageHeight}" w:lineRule="exact"/></w:pPr></w:p>`;
...
const cell = (width: number, row: number, imageHeight: number, imageAlignment: "left" | "center") => `<w:tc><w:tcPr><w:tcW w:w="${width}" w:type="dxa"/></w:tcPr>${row === 0 ? titleParagraph() : row === 1 ? imageParagraph(imageHeight, imageAlignment) : sourceParagraph()}</w:tc>`;
export function buildContainerOoxml(plan: ContainerPlan): string {
  ...
  const rows = Array.from({ length: 3 }, (_, row) => `<w:tr>${row === 1 ? `<w:trPr><w:trHeight w:val="${plan.imageRowHeightTwip}" w:hRule="atLeast"/></w:trPr>` : ""}${plan.columnWidthsTwip.map((w) => cell(w, row, plan.imageRowHeightTwip, plan.imageAlignment)).join("")}</w:tr>`).join("");
